...
class Host():
    ...
    def execute(self):
        ...
        for c in self.priority_list:
            ...
            #     lastMigrationTime += abs(
            #         self.env.hostlist[self.hostid].latency - self.env.hostlist[hostID].latency)  # abs获得绝对值
            # else:
            #     lastMigrationTime = 0
            # maxMigrationTime = max(maxMigrationTime)
            c_exeinfo = dict()
            container = self.env.getContainerByID(c)
            currentIPS = container.getBaseIPS()
            execTime = currentIPS / self.ipsCap
            exeTime += execTime
            c_exeinfo['container_id'] = c
            c_exeinfo['exe_time'] = execTime
            print(execTime)
