...
class COMALearner:
    def __init__(self, mac, scheme, logger, args):
        ...
        # copy.deepcopy 函数用于创建一个对象的深拷贝，这意味着它会递归地复制对象及其所有的子对象，从而生成一个完全独立的新对象
    ...
    def train(self, batch: EpisodeBatch, t_env: int, episode_num: int):
        ...
        mac_out = mac_out/mac_out.sum(dim=-1, keepdim=True)  # 去除掉不可用动作后重新归一化概率分布
        mac_out[avail_actions == 0] = 0  # 确保在归一化过程中，任何不可用动作的概率也不会被分配到
        ...
        q_taken = th.gather(q_vals, dim=1, index=actions.reshape(-1, 1)).squeeze(1)  # 根据 actions 中存储的动作索引，从 q_vals 中提取对应的Q值
        pi_taken = th.gather(pi, dim=1, index=actions.reshape(-1, 1)).squeeze(1)
        pi_taken[mask == 0] = 1.0  # 对无效动作设为1，在求对数概率时就变为0
        log_pi_taken = th.log(pi_taken)

        advantages = (q_taken - baseline).detach()

        coma_loss = - ((advantages * log_pi_taken) * mask).sum() / mask.sum()  # 乘mask是过滤无效的时间步
    ...
    def _train_critic(self, batch, rewards, terminated, actions, avail_actions, mask, bs, max_t):
        ...
        # Calculate td-lambda targets 从目标 Q 值中选择每个动作对应的 Q 值，通常用于计算损失或目标值。
